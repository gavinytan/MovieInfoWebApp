...
app.post('/api/mood', async(req,res) =>{
    ...
    let userInput = req.body.moods;
    let suggested_movie = []
    if (['happy', 'excited', 'joyful'].includes(userInput)){
    ...
    else if (['sad', 'depressed'].includes(userInput)){
        for(let i = 0; i < mood_result.data.items.length; i++){
    ...
    else if (['surprise', 'suspense', 'anxiety', 'dread', 'scare'].includes(userInput)) {
        for(let i = 0; i < mood_result.data.items.length; i++){
...
app.post('/api/trailers', async(req, res) => {
    ...
    res.status(200).send({link: t_link})
})
